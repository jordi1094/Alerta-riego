 ...
 * @param {UserRequestBody}userRequest - The user data to register, including name, username, email, and passwords.
 ...
 */
async function registerUser(userRequest: UserRequestBody, db: Sql<{}>): Promise<void | {error: Error}>{
    let cryptedPassword: string 
    if(userRequest.password === userRequest.passwordRepeat){
    cryptedPassword = await bcrypt.hash(userRequest.password, 12)  
    }else{
        return {error: new MatchError('password and password are diferents')}
    }
    
    const existingEmail = await  db`SELECT * FROM users WHERE email = ${userRequest.email}`
    if(existingEmail.length > 0){
        return ({error: new DupliciteError('Email already registered.')})
    }

    const existingUsername = await db`SELECT * FROM users WHERE username = ${userRequest.username}`
    if(existingUsername.length > 0){
        return ({error: new DupliciteError('Username already registered.')})
    }

    await db `INSERT INTO users (name, username, email, password)
        VALUES (${userRequest.name},${userRequest.username}, ${userRequest.email}, ${cryptedPassword})`
}
